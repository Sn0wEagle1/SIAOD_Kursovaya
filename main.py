...
def handle_type1_breaks(driver_state, current_time, is_peak, driver_id):
    ...
    if driver_state['worked_hours'] >= 6*60 and driver_state['has_rest'] != False:  # После 6 часов обязательно обед, чтобы не пропустить его
...
def handle_type2_breaks(driver_state, current_time, driver_id):
    # С вероятностью 0.4 водитель отрабатывает 4 часа и уходит на длинный перерыв
    if random.random() < 0.4 and driver_state['worked_hours'] >= 240:
        driver_state['had_long_break'] = True
        driver_state['rest_end_time'] = current_time + random.randint(30, 40)  # Длинный перерыв
        print(
            f"Водитель {driver_id + 1} (тип 2) завершил 4 часа работы в {format_time(current_time)} и уходит на длинный перерыв.")
        return random.randint(30, 40)

    # Если водитель отработал больше 6 часов, он обязан завершить 4 часа работы и уйти на длинный перерыв
    if driver_state['worked_hours'] >= 360:  # 6 часов работы
        if driver_state['worked_hours'] % 240 == 0:  # Если завершены полные 4 часа
            driver_state['had_long_break'] = True
            driver_state['rest_end_time'] = current_time + random.randint(30, 40)  # Длинный перерыв
            print(
                f"Водитель {driver_id + 1} (тип 2) отработал более 6 часов и завершает смену в {format_time(current_time)} с длинным перерывом.")
            return random.randint(30, 40)

    # Если водитель не имел длинного перерыва и отработал больше 4 часов
    if not driver_state['had_long_break'] and current_time > 13 * 60 and driver_state['worked_hours'] >= 240:
        driver_state['had_long_break'] = True
        driver_state['rest_end_time'] = current_time + random.randint(30, 40)  # Длинный перерыв
        print(
            f"Водитель {driver_id + 1} (тип 2) завершил маршрут в {format_time(current_time)} и уходит на длинный перерыв.")
        return random.randint(30, 40)

    # Если водитель отработал 2 часа с момента последнего перерыва
    if current_time - driver_state['rest_end_time'] >= 120:
        driver_state['rest_end_time'] = current_time + random.randint(15, 20)  # Короткий перерыв
        print(
            f"Водитель {driver_id + 1} (тип 2) завершил маршрут в {format_time(current_time)} и уходит на короткий перерыв.")
        return random.randint(15, 20)

    return 0


def handle_shift_change(driver_states, active_drivers, reserve_drivers, current_time, end_of_workday, driver_types=None):
    """
    Выполняет пересмену водителей с учетом их состояния, включая тип водителя, заданный заранее.
    """
    for bus_index, driver_id in enumerate(active_drivers[:]):  # Копируем список активных водителей
        driver_state = driver_states[driver_id]

        # Получаем тип водителя из переданного списка driver_types
        driver_type = driver_types[bus_index] if driver_types else (1 if driver_id % 2 == 0 else 2)

        # Определяем максимальную продолжительность смены в зависимости от типа водителя
        max_shift_duration = SHIFT_2_DURATION if driver_type == 2 else SHIFT_1_DURATION

        # Проверяем, нужно ли менять водителя
        if (
            driver_state['worked_hours'] >= max_shift_duration
            and (end_of_workday - current_time) > 60  # До конца дня должно оставаться более 1 часа
        ):
            ...
            for reserve_driver_id in reserve_drivers[:]:
                ...
                if (
                    current_time >= reserve_driver_state['rest_end_time']
                    and reserve_driver_state['rest_until_day'] <= (current_time // (24 * 60))  # Доступен по дням
                ):
                    # Выполняем пересмену
                    ...
                    driver_state['rest_until_day'] = (current_time // (24 * 60)) + (3 if driver_type == 2 else 1)
                    ...
                    print(f"Пересмена: водитель {driver_id + 1} (тип {driver_type}) заменен на {reserve_driver_id + 1}")
                    break
            else:
                # Нет доступных водителей для пересмены
                print(f"Пересмена невозможна для водителя {driver_id + 1} (тип {driver_type}): нет доступных резервных водителей")
...
def simulate_bus_route(bus_id, route, start_time, stops, drivers, driver_state, driver_type, is_peak):
    ...
    is_circular = route[0] == route[-1]  # Определяем, цикличный маршрут или нет
    ...
    for i in range(len(route) - 1):
        ...
        # Логика выхода пассажиров
        if is_circular:  # Для цикличного маршрута
            if i < len(route) - 1:  # На любой остановке кроме последней
                passengers_to_exit = int(total_passengers * 0.4)  # 40% выходят
                total_passengers -= passengers_to_exit
                remaining_capacity += passengers_to_exit
        else:  # Для конечного маршрута
            if i < (len(route) - 1) // 2:  # До середины маршрута
                passengers_to_exit = int(total_passengers * 0.4)  # 40% выходят
                total_passengers -= passengers_to_exit
                remaining_capacity += passengers_to_exit
            elif i == (len(route) - 1) // 2:  # В середине маршрута - конечная
                passengers_to_exit = total_passengers  # Все выходят
                total_passengers = 0
                remaining_capacity = BUS_CAPACITY

    # Все пассажиры выходят на последней остановке (если маршрут не цикличный)
    if not is_circular:
...
def genetic_algorithm(drivers, buses, points, all_routes, generations=30, population_size=20, mutation_rate=0.1):
    ...
    population = [
        (
            random.choices(all_routes, k=num_buses),  # Разрешаем повторяющиеся маршруты
            random.sample(range(len(drivers)), num_buses),  # Случайное распределение водителей
            [random.choice([1, 2]) for _ in range(num_buses)]  # Случайные типы водителей
        )
    ...
    for generation in range(generations):
        ...
        while len(new_population) < population_size:
            ...
            # Кроссинговер
            child_routes = parent1[0][:num_buses // 2] + parent2[0][num_buses // 2:]  # Маршруты
            child_drivers = parent1[1][:num_buses // 2] + parent2[1][num_buses // 2:]  # Водители
            child_types = parent1[2][:num_buses // 2] + parent2[2][num_buses // 2:]  # Типы водителей

            # Мутация
            if random.random() < mutation_rate:
                # Мутация маршрутов — допускаем повторяющиеся маршруты
                child_routes = random.choices(all_routes, k=num_buses)

            if random.random() < mutation_rate:
                # Мутация водителей
                child_drivers = random.sample(range(len(drivers)), num_buses)

            if random.random() < mutation_rate:
                # Мутация типов водителей
                child_types = [
                    random.choice([1, 2]) if random.random() < mutation_rate else driver_type
                    for driver_type in child_types
                ]

            new_population.append((child_routes, child_drivers, child_types))

        population = new_population

    # Находим лучшее решение
    best_fitness, best_individual = max(
        (calculate_fitness(individual, drivers, buses, points)[0], individual)
        for individual in population
    )

    # Извлекаем данные из best_individual
    best_solution = best_individual
    driver_types = best_individual[2]

    return best_solution, best_fitness, driver_types
...
def calculate_fitness(solution, drivers, buses, points):
    ...
    route_combination, driver_assignment, driver_types = solution
    total_passengers = simulate_short_week(drivers, buses, points, route_combination, driver_assignment, driver_types)
    return total_passengers, driver_types

# Главная функция симуляции
# Функция симуляции недели с учетом распределения маршрутов
def simulate_week(drivers, buses, points, route_combination=None, driver_assignment=None, driver_types=None):
    ...
    driver_types = driver_types[:] if driver_types else None  # Копируем driver_types
                ...
                for minute in range(6 * 60, 27 * 60):  # Симуляция рабочего дня
                    ...
                    for bus_id, route in enumerate(route_combination):
                        if bus_states[bus_id] <= minute:  # Если автобус свободен
                            ...
                            driver_type = driver_types[bus_id] if driver_types else (1 if driver_id % 2 == 0 else 2)
                            if not is_driver_available(driver_type, driver_state):
                                continue
        ...
        for minute in range(6 * 60, 27 * 60):  # Симуляция рабочего дня
            ...
            for bus_id, route in enumerate(routes):
                if bus_states[bus_id] <= minute:  # Если автобус свободен
                    driver_id = active_drivers[bus_id]
                    driver_state = driver_states[driver_id]
                    driver_type = driver_types[bus_id]
...
def run_genetic_algorithm():
    try:
        ...
        start_time = time.time()
        best_solution, best_fitness, driver_types = genetic_algorithm(drivers, buses, points, all_routes)
        ga_time = time.time() - start_time

        log_to_output(f"\nГенетический алгоритм: {best_fitness} пассажиров, время {ga_time:.2f} секунд")

        route_combination = all_routes[:len(buses)]
        driver_assignment = list(range(len(buses)))

        # Запуск симуляции недели
        schedule, total_revenue, best_combinations = simulate_week(
            drivers, buses, points, route_combination, driver_assignment, driver_types
        )
